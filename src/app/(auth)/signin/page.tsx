    ...
    try {
      ...
      if (typeof err === 'object' && err !== null && 'code' in err) {
        if (err.code === 'auth/invalid-credential') {
          setError(
            'Invalid credentials. Please check your email and password or sign up if you do not have an account yet.'
          );
        } else if (err.code === 'auth/user-not-found') {
          setError(
            'User with this email does not exist. Sign up to create an account.'
          );
      ...
      <div className="w-full max-w-md p-6 bg-white rounded-md shadow-md">
        ...
        <form onSubmit={handleSignIn} className="space-y-4">
          <div>
            <label
              htmlFor="email"
              className="block text-sm fontt-medium text-gray-700"
            >
              Email
            </label>
            <input
